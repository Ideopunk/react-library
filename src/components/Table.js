	...
	const booksTable = props.books.map((book) => {
		return (
			<div key={book.id} id={book.id} className="row">
					<div>{book.title}</div>
					<div>{book.author}</div>
					<img src={edit} className="edit-button row-button" onClick={() => props.initiateModify(book)} alt="edit"/>
					<div>{book.pages} pages</div>
					<div>{book.read}</div>
					<img src={trash} className="delete-button row-button" onClick={() => props.handleDelete(book.id)} alt="trash"/>
			</div>
		);
	});
	return (
